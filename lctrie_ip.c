...
init_reserved_subnets(lct_subnet_t prefix[],
  ...
  //
	prefix[4].info.type = IP_SUBNET_MULTICAST;
	inet_pton(AF_INET, "224.0.0.0", &(prefix[4].prefix));
	prefix[4].len = 4;

  // RFC 919/922, Section 7 Limited Broadcast Address
  //
	prefix[5].info.type = IP_SUBNET_BROADCAST;
	inet_pton(AF_INET, "255.255.255.255", &(prefix[5].prefix));
	prefix[5].len = 32;

  // RFC 1122, Sect. 3.2.1.3 Loopback
  //
	prefix[6].info.type = IP_SUBNET_LOOPBACK;
	inet_pton(AF_INET, "127.0.0.0", &(prefix[6].prefix));
	prefix[6].len = 8;
  ...
  //
	...
	prefix[7].info.rsv.desc = "RFC 1122, Sect. 3.2.1.3 \"This\" Networks";
	inet_pton(AF_INET, "0.0.0.0", &(prefix[7].prefix));
	prefix[7].len = 8;

  // RFC 5736 IETF Protocol Assignments
  //
	prefix[8].info.type = IP_SUBNET_RESERVED;
	prefix[8].info.rsv.desc = "RFC 5736 IETF Protocol Assignments";
	inet_pton(AF_INET, "192.0.0.0", &(prefix[8].prefix));
	prefix[8].len = 24;

  // RFC 5737 TEST-NET-1
  //
	prefix[9].info.type = IP_SUBNET_RESERVED;
	prefix[9].info.rsv.desc = "RFC 5737 TEST-NET-1";
	inet_pton(AF_INET, "192.0.2.0", &(prefix[9].prefix));
	prefix[9].len = 24;

  // RFC 3068 6to4 Relay Anycast
  //
	prefix[10].info.type = IP_SUBNET_RESERVED;
	prefix[10].info.rsv.desc = "RFC 3068 6to4 Relay Anycast";
	inet_pton(AF_INET, "192.88.99.0", &(prefix[10].prefix));
	prefix[10].len = 16;

  // RFC 2544 Network Interconnect Device Benchmark Testing
  //
	prefix[11].info.type = IP_SUBNET_RESERVED;
	prefix[11].info.rsv.desc = "RFC 2544 Network Interconnect Device Benchmark Testing";
	inet_pton(AF_INET, "198.18.0.0", &(prefix[11].prefix));
	prefix[11].len = 15;

  // RFC 5737 TEST-NET-2
  //
	prefix[12].info.type = IP_SUBNET_RESERVED;
	prefix[12].info.rsv.desc = "RFC 5737 TEST-NET-2";
	inet_pton(AF_INET, "198.51.100.0", &(prefix[12].prefix));
	prefix[12].len = 24;

  // RFC 5737 TEST-NET-3
  //
	prefix[13].info.type = IP_SUBNET_RESERVED;
	prefix[13].info.rsv.desc = "RFC 5737 TEST-NET-3";
	inet_pton(AF_INET, "203.0.113.0", &(prefix[13].prefix));
	prefix[13].len = 24;

  // RFC 1112, Section 4 Reserved for Future Use
  //
	prefix[14].info.type = IP_SUBNET_RESERVED;
	prefix[14].info.rsv.desc = "RFC 1112, Section 4 Reserved for Future Use";
	inet_pton(AF_INET, "240.0.0.0", &(prefix[14].prefix));
	prefix[14].len = 4;
